...
pub const PartitionProperties = struct {
    pub const equally = opencl.CL_DEVICE_PARTITION_EQUALLY;
    pub const by_counts = opencl.CL_DEVICE_PARTITION_BY_COUNTS;
    pub const by_counts_list_end = opencl.CL_DEVICE_PARTITION_BY_COUNTS_LIST_END;
    pub const by_affinity_domain = opencl.CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN;
};
pub const PartitionProperty = opencl.cl_device_partition_property;

pub const LocalMemType = enum(opencl.cl_device_local_mem_type) {
    local = opencl.CL_LOCAL,
    global = opencl.CL_GLOBAL,
};
...
pub fn createSubDevices(
    ...
    var out_devices_ptr: ?[*]DeviceId = null;
...
pub fn retain(device: DeviceId) OpenCLError!void {
    ...
    const errors_arr = .{ "out_of_host_memory", "invalid_device", "out_of_resources" };
    return errors.translateOpenCLError(errors_arr, ret);
}
...
pub fn release(device: DeviceId) void {
    ...
    const errors_arr = .{ "out_of_host_memory", "invalid_device", "out_of_resources" };
    std.debug.panic(
        "Unexpected error while releasing OpenCL device: {s}",
